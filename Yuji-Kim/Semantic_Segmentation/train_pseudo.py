...
from src.dataset import CustomDataLoader, AugmixDataLoader, PseudoDataset
...
def train(args):
    ...
    pseudo_dataset = PseudoDataset(data_dir='../input/data', pseudo_csv='pseudo_result.csv', transform=val_transform)
    ...
    first_cycle_steps = (len(train_loader) + len(pseudo_loader)) * args.epochs // args.scheduler_step
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        print("\nPseudo training start!")
        model.train()
        for step, (images, masks, _) in enumerate(tqdm(pseudo_loader)):
            ...
            if (step + 1) % args.log_step == 0:
                ...
                # wandb
                wandb.log({
                    "Pseudo loss" : train_loss / args.log_step,
                    "Pseudo mIoU" : train_iou / args.log_step
                })
        ...
        print(f"Current best miou is: {best_iou:.4f} at Epoch {best_epoch + 1}")
