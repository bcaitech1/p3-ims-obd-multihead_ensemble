...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, data_dir, cat_df, mode='train', num_cls=12, transform=None, augmix=None, prob=0.15):
        ...
        self.augmix = augmix
        self.tfms_b = A.Compose([
            A.Resize(256, 256),
            A.PadIfNeeded(512, 512, border_mode=0),
            A.ShiftScaleRotate(scale_limit=0, rotate_limit=[-30, 30], shift_limit=0.5, border_mode=0),
    ...
    def augmix_search(self, images, masks):
        ...
        if np.sum(augmix_mask != 0) > 20000:
            augmix_tfms = self.tfms_b
        elif np.sum(augmix_mask != 0) > 5000:
            augmix_tfms = self.tfms_m
        else:
            return images, masks

        transformed = augmix_tfms(image=augmix_img, mask=augmix_mask)
        augmix_img = transformed['image'].astype(np.uint8)
        augmix_mask = transformed['mask'].astype(np.uint8)
...
if __name__ == "__main__":
    ...
    tfms = ToTensorV2()
    ds = SegmentationDataset(train_annot_path, cat_df, mode='train', num_cls=12, transform=tfms, augmix=augmix_data,
                             prob=1.0)
