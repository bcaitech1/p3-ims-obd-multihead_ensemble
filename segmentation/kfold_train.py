...
def main():
    ...
    for fold_i in range(5):
        trn_annot = os.path.join(main_path, 'kfold_json', f'Fold_{fold_i}_trn.json')
        val_annot = os.path.join(main_path, 'kfold_json', f'Fold_{fold_i}_val.json')
        ...
        for epoch in range(args.epochs):
            ...
            version = f"{fold_i}_last"
            save_model(model, version=version, save_type='loss')
            if best_loss > val_loss:
                logger.info(f"Best loss {best_loss:.5f} -> {val_loss:.5f}")
                best_loss = val_loss
                version = f"{fold_i}_" + args.version
                save_model(model, version=version, save_type='loss')
            ...
            if best_mIoU < val_mIoU:
                ...
                best_mIoU = val_mIoU
                version = f"{fold_i}_" + args.version
                save_model(model, version=version, save_type='mIoU')
